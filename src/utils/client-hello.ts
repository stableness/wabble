...
function strip_sni_raw (chunk: Buffer): Buffer {
    ...
    return F.pipe(
        ...
        A.unfold(i, F.flow(
            O.fromPredicate(n => len_extensions.within(n)),
            O.map(extension(chunk)),
            O.map(ext => [ ext, ext.j ]),
        )),

        A.findFirst(({ id }) => id === 0x00), // ------- Extension: Server Name

        O.match(() => chunk, ({ i: n, total }) => {

            debug('Server Name', n, total, b => b.toString('utf8', 9));
...
const extension = (buf: Buffer) => (i: number) => F.pipe(

    S.get<number>(), S.bindTo('i'),
